...
def input_parser():
    ...
    parser.add_argument("--fname", type=str, default='../outputs/test_dsets.txt', help="The results file name")
    parser.add_argument("--calcsubset", action="store_const", default=True, const=True,
...
def parse_file(fname: str) -> Tuple[List[str], List[str], np.ndarray, Optional[np.ndarray]]:
    ...
    nbonds: Optional[np.ndarray] = None
    ignores = []
    if nentries == 110:
